...
# Modified by Jacob Freedman for Florida tweet data
...
#Load cleaned data
load(here("data","derived","private","florida_covid_cleaned.RData"))
load(here("data","derived","private","florida_baseline_cleaned.RData"))

############# TEMPORAL ANALYSIS ############# 

#create temporal data frame & graph it

floridaTweetsByHour <- ts_data(florida_covid, by="hours")
ts_plot(florida_covid, by="hours")
...
floridaNetwork <- network_graph(florida_covid, c("quote"))

plot.igraph(floridaNetwork)
...
floridaText <- florida_covid %>% 
  select(text, status_id) %>% 
  plain_tweets() 

# parse out words from tweet text
floridaWords = floridaText %>% unnest_tokens(word, text)
...
floridaWords =  floridaWords %>% anti_join(stop_words) 
...
floridaWordPairs %>%
  ...
  labs(title = "Word Network of Tweets about COVID-19 when mask mandate removed in Florida",
       x = "", y = "") +
  theme_void()
...
#creating a dataset with all the words and their sentiment values
florida_covid_sentiments <- left_join(floridaWords, get_sentiments("afinn"), by = "word") %>% 
  arrange(-value)

# scoring each tweet
florida_covid_sentiments_grouped <- florida_covid_sentiments %>% 
  group_by(status_id) %>% 
  summarize (value_int = sum(value, na.rm=TRUE))

# join tweet score back to each tweet
florida_covid <- left_join(florida_covid, florida_covid_sentiments_grouped, by = "status_id") 
...
counties <- get_estimates("county",
                          ...
                          key="df60b92b5b48ed43a3dbb334946f506f1a11ad26")
...
ggplot() +
  ...
  geom_point(data = florida_covid, aes(x=lng,y=lat),
